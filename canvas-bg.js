...
class Particle {
    ...
    draw() {
        ...
        ctx.fillStyle = '#79c0ff'; // Verde acento
        ctx.fill();
    }
}
    ...
    for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
            ...
            if (distance < maxDistance) {
                ...
                ctx.strokeStyle = `rgba(56, 139, 253, ${opacity * 0.25})`; // Verde 500
                ctx.lineWidth = 1;
                ctx.stroke();
            }
        }
...
let nodeColor = '#79c0ff';
let lineColor = '56, 139, 253';
